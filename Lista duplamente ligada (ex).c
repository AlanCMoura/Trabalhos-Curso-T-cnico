...
int inserirInicio(controleLista *lista, int elemento){ //Função de inserção no início da lista.
   ...
   if(listaVazia(lista) == 0){ //Se a lista estiver vazia: "proximo" de "novoNo" será nulo, e "novoNo" será igual a "inicio" e "fim" da lista.
       ...
       lista->inicio->anterior = novoNo; // O anterior do antigo "inicio" passa a ser "novoNo".
       lista->inicio = novoNo;
...
void mostrarLista(controleLista *lista){ //Função de exibição.
    ...
    no *atual = lista->fim;  //Um falso inicio é criado para percorrer toda a lista, sem modificá-la.
    
    while(atual != NULL){ //Enquanto o falso inicio for diferente de nulo, ele irá receber o valor de cada nó da lista.
